...
namespace Navalmod
{
    public class ArrowTurretBlock : MonoBehaviour

    {
        ...
        public void fireE()
        {
            try
            {
                ...
                {   
                    ...
                    {
                        if (!Physics.Raycast(CB.projectileSpawnPos.position, -CB.transform.up, salfefirerange.Value))
                        {
                            fire = true;
                            this.shootsound();
                            base.StartCoroutine(this.shoot());
        ...
        private void Update()
        {
            ...
            relodingtimeE -= Time.deltaTime;
            if ((StatMaster.InGlobalPlayMode || StatMaster.InLocalPlayMode || this.navalhe.open ))
            {


                if (acount <=0 && relodingE)
                {
                    relodingE = false;
                    relodingtime = relodtime.Value / navalhe.retime;
                }
                if (relodingtime < 0f)
                {
                    relodingE = true;
                    acount = (int)count.Value;
                }

                    if ((this.CB.FireKey.IsHeld || this.CB.FireKey.EmulationHeld(false) || AIfire) && relodingE == true && acount >= 0 && relodingtimeE < 0f)
                    {
                        ...
                        if (holdfire.IsActive)
                        {
                            ...
                            lightcountE++;
                            acount -= 1;
                            relodingtimeE = (relodtimeE.Value + UnityEngine.Random.Range(0f, ranrelod.Value)) / navalhe.retime;
                    ...
                    if (this.CB.FireKey.IsReleased || this.CB.FireKey.EmulationReleased())
                    {
                        this.canshoot = true;
                    }
                    if (ass == 1)
                ...
                try
                {
                    if (Vector3.Angle((-CB.transform.up.normalized * view.rangelast).normalized, ((view.lockg.transform.position - view.flakdpos) - CB.transform.position).normalized) < 5f && fireture && flaktext.Value != ""&& view.autofire.IsActive && view.locktype != 0)
                    {
                        AIfire = true;
                ...
                if (this.ass == 0)
                {
                    ...
                    acount = (int)count.Value;
                    this.ass = 1;
                    lightcount = 0.2f/relodtimeE.Value;
                    lightcountE = 0f;
                    CB.projectileSpawnPos.transform.position += base.transform.up.normalized * -laught.Value;
                    SafeAwakeE();
        ...
        public void SafeAwakeE()
        {
            try
            {
                this.MyGUID = base.GetComponent<BlockBehaviour>().BuildingBlock.Guid.ToString();
                this.CB = base.GetComponent<CrossBowBlock>();
        ...
        {
            this.CB = base.GetComponent<CrossBowBlock>();
            count = CB.AddSlider("单个弹夹炮弹数量", "count", 100f, 1f, 500f, "", "x");
            laught = CB.AddSlider("弩箭出生点位置", "laught", 1f, 0f, 10f, "", "x");
            flaktext = CB.AddText("火控接口编号", "flaktext", "");
            holdfire = CB.AddToggle("按住开火", "holdfire", true);
            ranrelod = this.CB.AddSlider("随机延迟(s)", "relodtimeS", 0f, 0f, 2f, "", "x");

            shootspeed = this.CB.AddSlider("弹速（m/s）", "shootspeed", 600f, 0f, 1000f, "", "x");

            this.mass = this.CB.AddSlider("弹重（kg）", "mass", 1f, 0f, 2000f, "", "x");
            this.scale = this.CB.AddSlider("口径", "scaleE", 25f, 40f, 460f, "", "x");
            this.tntmass = this.CB.AddSlider("装药量（kg）(HE)", "tntmass", 0.05f, 0f, 70f, "", "x");
            relodtime = this.CB.AddSlider("装填时间(s)", "relodtime", 20f, 0.5f, 40f, "", "x");
            relodtimeE = this.CB.AddSlider("开炮间隔(s)", "relodtimeE", 0.5f, 0.05f, 1f, "", "x");
            this.velocityattenuation = this.CB.AddSlider("速度衰减", "velocityattenuation", 0.1f, 0.01f, 0.5f, "", "x");

            trail = CB.AddMenu("trailfuse", 0, new List<string>() { "曳光尾迹", "烟雾尾迹", "水尾迹" }, false);
            this.backlash = this.CB.AddSlider("后座力", "backlash", 0f, 0f, 1f, "", "x");
            ...
            int n = 0;
            CB.OnReloadAmmo(ref n, AmmoType.All, true, true);
        ...
        private IEnumerator shoot()
        {
            try
            {
                ...
                try
                {
                    ...
                    cannonball.transform.position = this.CB.projectileSpawnPos.position;
                    ...
                    if (lightcountE > lightcount)
                    {
                        ...
                        int ccount = this.nAbolts.Count - 1;
                        nAbolt.ccount = ccount;

                        if (StatMaster.isHosting)
                        {

                            Message cannonfire = Messages.ballshootE.CreateMessage(new object[]
                                {
                    this.MyGUID,
                    ccount,
                    canball.transform.position,
                    rigidbody.velocity,
                    (int)CB.ParentMachine.PlayerID
                                });
                    ...
                    {
                        canball.GetComponent<TrailRenderer>().enabled = false;
                    }
